...
int main()
{
    ...
    //向文件写入用户名
    //int ret = write(fd,test.username,strlen(test.username)+1);//函数strlen
    ...
    //向文件写入salt
    //ret=write(fd,test.salt,strlen(test.salt)+1);
    ...
    //向文件写入加密后密文
    //ret=write(fd,test.dcrypt,strlen(test.dcrypt)+1);
    //printf("dcrypt_size= %d\n",sizeof(dcrypt));
